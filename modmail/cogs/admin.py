...
class Admin(commands.Cog):
    ...
    @checks.is_admin()
    @commands.command(description = "Get a list of servers with the specified name.", usage = "findserver <name>", hidden = False)
    ...
    @checks.is_admin()
    @commands.command(description = "Get a list of servers the bot shares with the user.", usage = "sharedservers <user>", hidden = False)
    ...
    @checks.is_admin()
    @commands.command(description = "Create an invite to the specified server.", usage = "createinvite <server ID>", hidden = False)
    async def createinvite(self, ctx, *, guild: converters.GlobalGuild):
        ...
        if not invite:
            ...
            await ctx.send(embed = discord.Embed(description = f"Here is the invite link: https://discord.gg/{invite.code}", colour = self.bot.primary_colour))

    @checks.is_admin()
    @commands.command(description = "Get the top servers using the bot.", aliases = ["topguilds"], usage = "topservers [count]", hidden = True)
    ...
    @checks.is_admin()
    @commands.command(description = "Make me say something.", usage = "echo [channel] <message>", hidden = False)
    ...
    @checks.is_admin()
    @commands.command(description = "Get clusters' statuses.", usage = "botstatus", hidden = False)
    async def botstatus(self, ctx):
        ...
        for x in data:
            clusters[x[0]] = x[1]
        await ctx.send(
            embed=discord.Embed(description = f"```json\n{json.dumps(clusters, indent = 4, default = lambda o: o.__dict__)}```", colour = self.bot.primary_colour))
