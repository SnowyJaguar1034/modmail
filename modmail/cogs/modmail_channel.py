...
class ModMailEvents(commands.Cog):
    ...
    async def send_mail_mod(self, message, prefix, anon: bool = False, msg: str = None, snippet: bool = False):
        ...
        try:
            ...
            embed.set_footer(text=f"{member} | {member.id}", icon_url=member.avatar_url)            for count, attachment in enumerate([attachment.url for attachment in message2.attachments], start=1):
