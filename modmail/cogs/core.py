...
class Core(commands.Cog):
    ...
    async def close_channel(self, ctx, reason, anon: bool = False,):
        try:
            ...
            embed = discord.Embed(
                title="Ticket Closed",
                description=(reason if reason else discord.Embed.Empty),
                colour=self.bot.error_colour,
                timestamp=datetime.utcnow(),
            )
            embed.set_author(
                name=str(ctx.author.name) if anon is False else "GOFS#0000",
            ...
            if member:
                try:
                    ...
                    if data[6]:
                        embed.add_field(
                            name="Custom Closing Message",
                            value=self.bot.tools.tag_format(data[6], member),
                        )
    ...
    @commands.guild_only()
    @commands.command(description="Close the channel anonymously.", usage="aclose [reason]")
    async def edit(self, ctx, postlocal: discord.Message, *, text: str):
        data = await self.bot.get_data(ctx.guild.id)
        tickets = await self.bot.get_tickets(postlocal.id)
        postremote = await self.bot.get_message(tickets[2])
        modmail_log = await self.bot.get_channel(id = data[4])
        embededit = discord.Embed(
            title = "Ticket Message Edit!",
            description = f"Ticket: {ctx.channel.mention} | {ctx.channel.name} | {ctx.channel.id}",
            timestamp = datetime.datetime.utcnow()
            )
        embededit.set_author(name = ctx.author, icon_url = ctx.author.avatar_url)
        embededit.add_field(name = "Old message:", value = postlocal.embeds[0].description)
        embededit.add_field(name = "New message:", value = text)
        embededit.set_footer(text=postlocal.embeds[0].footer.text, icon_url = postlocal.embeds[0].footer.icon_url)
        embedlocal = discord.Embed(
            title = postlocal.embeds[0].title,
            description = text,
            colour = postlocal.embeds[0].colour,
            timestamp = postlocal.embeds[0].timestamp
        )
        embedlocal.set_footer(text=postlocal.embeds[0].footer.text, icon_url = postlocal.embeds[0].footer.icon_url)
        embedremote = discord.Embed(
            title = postremote.embeds[0].title,
            description = text,
            colour = postremote.embeds[0].colour,
            timestamp = postlocal.embeds[0].timestamp
            )
        embedremote.set_footer(text=postremote.embeds[0].footer.text, icon_url = postremote.embeds[0].footer.icon_url)
        await postremote.edit(embed=postremote)
        await postlocal.edit(embed=postlocal)
        await modmail_log.send(embed=embededit)
    ...
    @commands.guild_only()
    @commands.command(description="Close the channel anonymously.", usage="aclose [reason]")
    async def delete(self, ctx, postlocal: discord.Message):
        data = await self.bot.get_data(ctx.guild.id)
        tickets = await self.bot.get_tickets(postlocal.id)
        modmail_log = await self.bot.get_channel(id = data[4])
        postremote = await self.bot.get_message(tickets[2])
        embeddelete = discord.Embed(
            title = "Ticket Message Delete!",
            description = f"Ticket: {ctx.channel.mention} | {ctx.channel.name} | {ctx.channel.id}",
            timestamp = datetime.datetime.utcnow()
            )
        embeddelete.set_author(name = ctx.author, icon_url = ctx.author.avatar_url)
        await postremote.delete()
        await postlocal.delete()
        await modmail_log.send(embed=embeddelete)
