...
class Owner(commands.Cog):
    ...
    @checks.is_owner()
    @commands.command(description="Load a module.", usage="load <cog>", hidden=False)
    ...
    @checks.is_owner()
    @commands.command(description="Unload a module.", usage="unload <cog>", hidden=False)
    ...
    @checks.is_owner()
    @commands.command(description="Reload a module.", usage="reload <cog>", hidden=False)
    ...
    @checks.is_owner()
    @commands.command(name="eval", description="Evaluate code.", usage="eval <code>", hidden=False)
    ...
    @checks.is_owner()
    @commands.command(description="Evaluate code on all clusters", usage="evall <code>", hidden=False)
    ...
    @checks.is_owner()
    @commands.command(description="Execute code in bash.", usage="bash <command>", hidden=False)
    ...
    @checks.is_owner()
    @commands.command(description="Execute SQL.", usage="sql <query>", hidden=False)
    ...
    async def givepremium(self, ctx, user: converters.GlobalUser, *, expiry: converters.DateTime):
        ...
        if premium:
            await ctx.send(embed=discord.Embed(description="That user already has premium.", colour=self.bot.error_colour))
            return
        async with self.bot.pool.acquire() as conn:
            timestamp = int(expiry.replace(tzinfo=timezone.utc).timestamp() * 1000)
            await conn.execute("INSERT INTO premium (identifier, guild, expiry) VALUES ($1, $2, $3)", user.id, [], timestamp)
        await ctx.send(embed=discord.Embed(description="Successfully assigned that user premium temporarily.", colour=self.bot.primary_colour,), content=f"<t:{int(timestamp)}:R>")
    ...
    @checks.is_owner()
    @commands.command(description="Ban a user from the bot", usage="banuser <user>", hidden=False)
    ...
    @checks.is_owner()
    @commands.command(description="Unban a user from the bot", usage="unbanuser <user>", hidden=False)
